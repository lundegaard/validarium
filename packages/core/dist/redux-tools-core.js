(function(global, factory) {
	typeof exports === 'object' && typeof module !== 'undefined'
		? factory(exports, require('invariant'), require('ramda-extension'), require('./utils'))
		: typeof define === 'function' && define.amd
			? define(['exports', 'invariant', 'ramda-extension', './utils'], factory)
			: factory((global.ReduxToolsCore = {}), global.invariant, global.R_, global.lndCardifUtils);
})(this, function(exports, invariant, ramdaExtension, lndCardifUtils) {
	'use strict';
	...
	const isIntlMessage = function isIntlMessage(x) {
		return x && ramdaExtension.notNil(x.id) && ramdaExtension.notNil(x.defaultMessage);
	};

	const notNilObject = allPass([ramdaExtension.notNil, ramdaExtension.isObject]);

	const isIntlMessageWithArgs = allPass([
	...
	const pickByNotNil = pickBy(ramdaExtension.notNil);
	/**
	 * Recursevily translates messages in validation result.
	 */

	const translate = function translate(formatMessage) {
		return cond([
			[
				...
				function(_ref2) {
					...
					const messageValues = _ref2.messageValues;
			...
			[
				notNilObject,
				map(
					lndCardifUtils.unlessIsNil(function(x) {
	...
	const existFormatMessage = o(ramdaExtension.isFunction, path(['intl', 'formatMessage']));
	/**
	 * For merging validation results.
	 * We don't want to null results replacing those already filled.
	 */

	const mergeResults = ramdaExtension.mergeDeepAllWith(
		cond([
			[lndCardifUtils.isNilOrAllIsNil, nthArg(1)],
			[flip(lndCardifUtils.isNilOrAllIsNil), ramdaExtension.headArg],
			[
				function(a, b) {
	...
	const createMainValidate = function createMainValidate() {
		...
		return function(props) {
			return function(values) {
				...
				return compose(
					pickByNotNil,
					lndCardifUtils.unlessIsNil(translate(props.intl.formatMessage)),
					mergeResults,
	...
	const dispatchValidPredicates = function dispatchValidPredicates(predicates) {
		return function() {
	...
	const validateObjectDescriptor = function validateObjectDescriptor(descriptor) {
		const evolution = map(dispatchValidPredicates)(descriptor);

		const keysToValidate = keys(descriptor);

		const dull = map(ramdaExtension.alwaysNull)(descriptor);
	...
	const validateListDescriptor = function validateListDescriptor(listDescriptor) {
	...
	const validate = ifElse(ramdaExtension.isArray, validateListDescriptor, validateObjectDescriptor);
	...
	const validateMany = useWith(map, [validate, ramdaExtension.defaultToEmptyArray]);
	...
	const createValidation = function(predicate, message, messageValues) {
		return ifElse(
	...
	const createOptionalValidation = function(predicate, message, messageValues) {
		return ifElse(
			predicate,
			ramdaExtension.alwaysNull,
			always({
